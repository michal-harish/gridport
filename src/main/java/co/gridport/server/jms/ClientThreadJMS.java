...
public class ClientThreadJMS extends ClientThread {
    ...
    private co.gridport.server.jms.ModuleJMS jmsModule;

    public ClientThreadJMS(RequestContext context) {
        super(context);
        this.jmsModule = GridPortServer.getModule(ModuleJMS.class);
    }
    ...
    public void execute() throws InterruptedException {
        ...
        if (request.getMethod().equals("PUT")) {
            ...
            String error = jmsModule.addSubscriber(destination, target);
            try {
                if (error == null) {
        ...
        if (request.getMethod().equals("DELETE")) {
            String target = new String(this.body);
            String error = jmsModule.removeSubscriber(destination, target);
            try {
                if (error == null) {
